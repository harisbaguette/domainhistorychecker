...
- 복사일: 2026-08-06 (모바일 뼈대 이식) · **2026-08-22 `tokens.css` `base.css` 다시 복사**
  (정본이 2026-08-10 에 손글씨를 BM 연성체로 갈아타고 고정폭 토큰 `--dw-font-mono` 를 새로 넣었다.
  나머지 15장은 그때도 지금도 정본과 한 글자도 다르지 않다 — 파일별로 해시를 맞춰 확인했다.)
...
- `base.css`의 `@font-face`는 손글씨 폰트(`Yeon Sung` · BM 연성체) woff2를 자기 폴더 기준
  상대경로(`../assets/fonts/`)로 가리킨다. 이 앱은 글꼴을 `static/fonts/` 에 두므로 그 경로가 맞지 않는다.
  `base.css` 자체는 한 글자도 고치지 않고, `static/app.css` 가 **다른 이름**(`DoweekHand`)으로
  이 앱의 글꼴을 다시 선언해 그 자리를 대신한다 — 같은 이름을 쓰면 브라우저가 `base.css` 의
  없는 경로까지 받으러 가 404가 난다(실측 404 0건).
  이름이 서로 다르면 `base.css` 의 선언은 아무도 안 쓰는 채로 남고, 안 쓰이는 글꼴은
  브라우저가 아예 받으러 가지 않으므로 없는 경로라도 404가 나지 않는다.
