...
- 글꼴 파일은 `static/fonts/MemomentKkukkukk.woff2`(1.9MB · 한글 음절 11,172자 + 라틴 전부).
  ...
  이름을 `DoweekHand`로 지은 것은 `base.css`의 손글씨 선언과 이름이 겹치면 브라우저가 거기 적힌
  (이 앱에 없는) 경로까지 받으러 가 404가 나기 때문이다 — 이름을 가른 뒤 실측 404 0건.
  2026-08-22에 DW 정본이 손글씨를 BM 연성체(`Yeon Sung`)로 갈아탄 것을 그대로 받아 왔다.
  이름이 아예 달라져서 겹칠 일이 더 없어졌고, `app.css`가 `--dw-font-*`를 뒤에서 덮으므로
  `base.css`의 연성체 선언은 쓰이지 않는다(안 쓰이는 글꼴은 브라우저가 받으러 가지 않는다).
