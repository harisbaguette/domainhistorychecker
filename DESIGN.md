...
- **필터 탭**: 흰 띠(모서리 14px, 안쪽 4px) 안에서 색칠된 알약이 고른 칸으로 미끄러지고(0.18s),
  ...
  갈래는 검사 흐름 그대로다 — 2026-08-22에 검사가 깔때기 4단으로 바뀌었지만 갈래 이름은 그대로 맞는다:
  1단 주인 여부 → 2단 공개 위험 명단(공짜) → 3단 가장 최근 화면 한 장 + 블랙리스트·기록 규모(싼 조회)
  → 4단 옛 화면 전수 정독(비싼 것) → 4단을 지난 것만 매입 후보. 걸러진 것은 "주인 있음"·"스팸" 갈래로 들어간다.
...
| 진행률 | `progress` | `role="progressbar"` + 진행 문구를 `aria-labelledby` 로 가리킨다. 진행 문구 맨 앞에는 지금 어느 단인지가 붙는다(`1단 ·` / `2단 ·` / `3단 ·` / `4단 ·`) — 같은 "검사 중"이라도 몇 초짜리(1~3단)인지 몇 분짜리(4단)인지가 이 한 조각으로 갈린다(2026-08-22 깔때기 4단 재배치) |
| 결과 없음 | `empty-state` | |
