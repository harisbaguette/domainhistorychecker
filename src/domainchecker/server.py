...
# 4단 깔때기의 바닥값 — 3단에서 끝나는 도메인이 웨이백에 쓰는 횟수는 목록 조회 한 번뿐이다.
# 주인이 있거나(1단) 위험 명단에 걸린(2단) 도메인은 이마저도 0회지만, 몇 개가 그리 될지는
# 미리 알 수 없어 바닥값에는 세지 않는다(실제는 이보다 짧아진다).
...
def estimate(config: Config, count: int) -> dict:
    ...
    깔때기라서 도메인마다 드는 값이 다르다 — 3단에서 끝나면 웨이백 한두 번, 4단까지
